...
function M.open(pr, files, opts)
  ...
  if not ok then
    vim.notify("gh-review: requires snacks.nvim (https://github.com/folke/snacks.nvim)", vim.log.levels.ERROR)
    return
  end

  local state_mod = require("gh-review.state")
  ...
  require("gh-review.ui.layout").open(state, config)
  ...
  if #files > 0 then
    ...
    vim.notify("gh-review: PR #" .. pr.number .. " has no changed files", vim.log.levels.INFO)
  end
end
